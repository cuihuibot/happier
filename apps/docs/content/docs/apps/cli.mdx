...
Development builds also provide `happier profiles set` for saved native-worker
mappings and account/parent delegation preferences, and `happier session run start
--profile <id-or-name>` for managed starts. See [Saved native workers](/agents/profiles#saved-native-workers-development)
for the required experimental feature, matching-host restriction, setup and
unverified live-compatibility limits. No native definitions are installed by these
commands.
