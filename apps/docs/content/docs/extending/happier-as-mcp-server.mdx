...
Call `execution_run_start` to start a structured Happier-managed run in a session. It requires an `intent` plus a backend target, or a saved native-worker `profileId` for a non-voice run.
...
On builds supporting native-worker profiles, an existing saved profile supplies the backend and launch defaults:

```json
{
  "intent": "delegate",
  "profileId": "saved-worker",
  "instructions": "Review the proposed change and report concrete risks."
}
```

In a session-agent call, omit `sessionId` to use the invoking session. Profiles do not grant permissions beyond the parent session's allowed permissions.
