...
Development builds can also use a saved account or parent-profile Happier route.
The route is applied to a **fresh parent**, not retroactively to an existing
conversation; explicit current instructions and permission policy still win.
See [Parent delegation preference](/agents/profiles#parent-delegation-preference-development).
This is routing guidance, not enforcement that disables an Agent's native tools.
