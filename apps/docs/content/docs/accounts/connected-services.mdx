...
GitHub tokens can be used by source-control features such as repository publishing and pull request operations. A selected GitHub token profile can also authenticate new Copilot sessions.
...
It can also authenticate Copilot:

- choose a connected GitHub token profile when starting a Copilot session
- Happier provides that token to the new Copilot process as `COPILOT_GITHUB_TOKEN`
- the token value is not stored in session metadata

<Callout type="info">
  Copilot connected-service support is currently available only in development
  builds. It has not been included in a published release yet.
</Callout>
...
- Copilot accepts `github` token profiles only. GitHub OAuth credentials are not supported in this build.
